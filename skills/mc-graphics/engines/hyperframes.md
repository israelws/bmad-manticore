...
- Installed at setup by mc-setup, so the whole capability surface is known from the beats stage onward, not just at graphics time: `npx skills add heygen-com/hyperframes --all --full-depth` loads the whole catalog (or `npx hyperframes skills update` for the maintained core set). Favor them: the installed skills plus the block catalog are the FIRST place to look for any beat, ahead of authoring anything by hand.
